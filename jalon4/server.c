...
// inistialiser le pointeur de contrôl de la chaine des clients
...
void supprimer(Client_ancien *Client_ancien,salon_recent* Salon_recent, int Client_fd )
{
	...
	if ( Client_ancien->premier != NULL){
		...
		{	
			static int c = 0;
			if (Client_actuel->client_fd == Client_fd && c == 0 && Client_actuel->suivant->suivant != NULL){ // si le 1er de la liste chainé qu'on veut supprimer
		...
		for (int i = 0; i < nfds; i++) {
			...
			// read data from socket
				...
				if (strcmp(buff,"/quit\n") == 0) { // If a socket previously created to communicate with a client detects a disconnection from the client side
                    ...
                    char test2[MSG_LEN];// store changed pseudo or data we need to empty the buff
                    char test[MSG_LEN]; // store data we need to empty the buff
                    memset(test,0,MSG_LEN);
					memset(test2, 0, MSG_LEN);
                    int dont_send_back = 0; // to avoid having msg back
					...
					case UNICAST_SEND :
						...
						extract_data(0,strlen(buff)-1,buff,test2);
						memset(buff, 0, MSG_LEN);
                    ...
                    case FILE_REQUEST :
						memset(test,0,MSG_LEN); 
						extract_data(0,strlen(buff)-1,buff,test);// copy file name
                        memset(test2,0,MSG_LEN);
						strcpy(test2,msgstruct.nick_sender);
						strcpy(test2,strcat(test2," wants you to accept the transfer of the file named \""));
						strcpy(test2,strcat(test2,test));
                        strcpy(test2,strcat(test2,"\"."));
                        memset(buff, 0, MSG_LEN);

						strcpy(buff,unicast_send(Client_ancien,test2,msgstruct)); // send msg
                        if ( (strcmp(buff,"User not found :/\n")) == 0){ // If the pseudo doesn't exist send a msg back
                            msgstruct.type = NICKNAME_INFOS; 
                        }
                        else{
                            dont_send_back = 1; // don't send msg back
                        }
						msgstruct.pld_len = strlen(buff);

						break;
                    case FILE_ACCEPT :
						unicast_send(Client_ancien,buff,msgstruct);
                        dont_send_back = 1;
						break;
                    case FILE_REJECT :
						unicast_send(Client_ancien,buff,msgstruct);
					...
					if (msgstruct.type != NICKNAME_NEW && msgstruct.type != MULTICAST_SEND && dont_send_back == 0){ // le cas de NICKNAME_NEW est traité dans Echo_for_pseudo
					// send back data to client
						// Sending structure 
